...
const feishuShareSdk = (config: FeishuShareSdkConfig): FeishuShareAction => {
    ...
    const init = () => {
        return new Promise((resolve, reject) => {
            // @ts-ignore 例如:
            if (window.h5sdk) {
                if (cache.latestTime && Date.now() - cache.latestTime < 30 * 60 * 1000) {
                    resolve(true);
                } else {
                    const params = urlJoinParams({
                        "app_id": app_id,
                        "app_secret": app_secret
                    })
                    const get_tenant_access_token = async () => {
                        const res = await fetch(
                            `${proxy_prefix}/open-apis/auth/v3/tenant_access_token/internal${params}`,
                            {
                                headers: {
                    ...
                    const get_ticket = async (access_token: string) => {
                        ...
                        const res = await fetch(
                            `${proxy_prefix}/open-apis/jssdk/ticket/get${params}`,
                            {
                                method: 'POST',
                                headers: {
                                    "Content-Type": 'application/json; charset=utf-8',
